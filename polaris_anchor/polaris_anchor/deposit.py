...
from .sep24 import forms as form2
...
class AnchorDeposit(DepositIntegration):
    def form_for_transaction(
        ...
        **kwargs
        ...
        if not user:
            if post_data:
                return form2.ContactForm(post_data)
            else:
                return form2.ContactForm()
        # if we haven't gotten the user's full address, colelct it
        elif not user.full_address:
            if post_data:
                return form2.AddressForm(post_data)
            else:
                return form2.AddressForm()
    ...
    def after_form_validation(
        ...
        **kwargs,
        ...
        if isinstance(form, TransactionForm):
            ...
            return
        if isinstance(form, form2.ContactForm):
            # creates the user to be returned from user_for_account()
            create_user(form)
        elif isinstance(form, form2.AddressForm):
